...
//			return new ResponseEntity<>(ticket, HttpStatus.OK);
	...
	public ResponseEntity<User> findUserByEmail(@PathVariable String token) {
		...
		return new ResponseEntity<>(userService.findByEmail(claim.getSubject()), HttpStatus.OK);
	}
	
	@PutMapping("/update")
	public ResponseEntity<?> updateUser(@RequestBody User user) throws UserNotExistsException {
		System.out.println(user);
		try {
			return new ResponseEntity<>(userService.updateUser(user), HttpStatus.BAD_REQUEST);
	...
	public ResponseEntity<?> deleteUser(@PathVariable String token) throws UserNotExistsException {
		Claims claim = Jwts.parser().setSigningKey("wipro123").parseClaimsJws(token).getBody();
		System.out.println(claim.getSubject());
		try {
			return new ResponseEntity<>(userService.deleteUser(claim.getSubject()), HttpStatus.BAD_REQUEST);
		} catch (UserNotExistsException e) {
			throw new UserNotExistsException();
		}catch(Exception e) {
			return new ResponseEntity<String>("Internal Server Error : "+e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
